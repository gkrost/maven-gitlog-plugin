...
public class SimpleHtmlRenderer extends FileRenderer {
	...
	public SimpleHtmlRenderer(Log log, File targetFolder, String filename, boolean fullGitMessage,
			MessageConverter messageConverter, boolean tableOnly) throws IOException {
	...
	public void renderHeader(String reportTitle) throws IOException {
		...
		tableHtml.append("\t<table class=\"changelog\">").append(NEW_LINE).append("\t\t<tbody>").append(NEW_LINE);
	}

	@Override
	public void renderTag(RevTag tag) throws IOException {
		tableHtml.append("\t\t<tr class=\"tag\"><td colspan=\"3\">")
				.append(SimpleHtmlRenderer.htmlEncode(tag.getTagName())).append("</td></tr>").append(NEW_LINE);
	}
	...
	public void renderCommit(RevCommit commit) throws IOException {
		...
		String authorHtml = "<span class=\"committer\">" + Formatter.formatCommiter(commit.getCommitterIdent())
				+ "</span>";
		if (!areSame(author, committer)) {
			authorHtml = "<span class=\"author\">" + author + "</span> by " + authorHtml;
		}

		tableHtml.append("\t\t<tr>").append("<td class=\"date\">").append(date).append("</td>").append("<td>")
				.append(authorHtml).append("</td>").append("<td>").append(message).append("</td>").append("</tr>")
				.append(NEW_LINE);
	}
	...
	public void renderFooter() throws IOException {
		tableHtml.append("\t\t</tbody>").append(NEW_LINE).append("\t</table>").append(NEW_LINE);

		if (tableOnly) {
			writer.append(tableHtml.toString());
		} else {
			String html = template.replace("{title}", htmlEncode(title)).replace("{table}", tableHtml.toString());
			writer.append(html);
		}
	}
