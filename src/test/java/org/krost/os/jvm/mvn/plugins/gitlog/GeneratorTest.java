...
import static org.krost.os.jvm.mvn.plugins.gitlog.renderers.JiraIssueLinkConverterTest.JIRA_PATTERN;
...
public class GeneratorTest {
	private static final String THIS_PLUGIN_ISSUES = "https://github.com/gkrost/maven-gitlog-plugin/issues/";
	...
	public void writePlainTextFullLogToFileCustomDir() throws Exception {
		Log log = new SystemStreamLog();
		ChangeLogRenderer renderer = new PlainTextRenderer(log, new File(TARGET_DIR), "changelogFullCustomDir.txt",
				true);
	...
	public void writeSimpleHtmlLogToFile() throws Exception {
		...
		ChangeLogRenderer renderer = new SimpleHtmlRenderer(log, new File(TARGET_DIR), "changelog.html", false,
	...
	public void writeSimpleHtmlFullLogToFile() throws Exception {
		Log log = new SystemStreamLog();
		GitHubIssueLinkConverter messageConverter = new GitHubIssueLinkConverter(log, THIS_PLUGIN_ISSUES);
		ChangeLogRenderer renderer = new SimpleHtmlRenderer(log, new File(TARGET_DIR), "changelogFullMessage.html",
				true, messageConverter, false);
		generateReport(log, renderer);
	}
	...
	public void writeHtmlTableOnlyLogToFile() throws Exception {
		Log log = new SystemStreamLog();
		GitHubIssueLinkConverter messageConverter = new GitHubIssueLinkConverter(log, THIS_PLUGIN_ISSUES);
		ChangeLogRenderer renderer = new SimpleHtmlRenderer(log, new File(TARGET_DIR), "changelogtable.html", false,
	...
	public void writeMarkdownLogToFile() throws Exception {
		Log log = new SystemStreamLog();
		GitHubIssueLinkConverter messageConverter = new GitHubIssueLinkConverter(log, THIS_PLUGIN_ISSUES);
		ChangeLogRenderer renderer = new MarkdownRenderer(log, new File(TARGET_DIR), "changelog.md", false,
				messageConverter, false);
		generateReport(log, renderer);
	}
	...
	public void writeAsciidocLogToFile() throws Exception {
		Log log = new SystemStreamLog();
		GitHubIssueLinkConverter messageConverter = new GitHubIssueLinkConverter(log, THIS_PLUGIN_ISSUES);
		ChangeLogRenderer renderer = new AsciidocRenderer(log, new File(TARGET_DIR), "changelog.adoc", false,
				messageConverter, null, false, null, null);
		generateReport(log, renderer);
	}
	...
	public void writeAsciidocLogToFileJiraAsTableView() throws Exception {
		...
		ChangeLogRenderer renderer = new AsciidocRenderer(log, new File(TARGET_DIR), "changelog-jira-asTableview.adoc",
				false, messageConverter, "==", true, "Date", "Commit");
		generateReport(log, renderer);
	}
	...
	private void generateReport(Log log, ChangeLogRenderer renderer, File gitdir)
			throws IOException, NoGitRepositoryException {
		generateReport(log, renderer, null, Defaults.COMMIT_FILTERS);
	}

	private void generateReport(Log log, ChangeLogRenderer renderer, File gitdir, List<CommitFilter> filters)
			throws IOException, NoGitRepositoryException {
